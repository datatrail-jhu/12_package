...
{choose-answers: 4} 
? Which datasets within packages do we document?

C) external datasets made available to the user
o) internal datasets utilized by functions
o) internal datasets used by other unit tests
o) raw datasets prior to cleaning
o) the code used to clean the raw dataset

{choose-answers: 4} 
? Where in your package are data files stored if you want them to be available to users?
...
{choose-answers: 4} 
? Which additional roxygen2 tag should you use when documenting a dataset (that you don't use when documenting functions)?
...
{choose-answers: 4} 
? Which roxygen2 tag should you *never* use when documenting a dataset?
...
{choose-answers: 4} 
? Which line of code below would require you to document a dataset?

C) `usethis::use_data(df)`
o) `usethis::use_data_raw("df")`
o) `system.file("extdata", "data.csv", package = "cbds")`
o) `usethis::use_data(df, internal = TRUE)`
o) `usethis::use_data(df, external = TRUE)`
o) `usethis::use_data(df, external = FALSE)`
